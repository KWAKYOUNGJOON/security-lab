...
class Phase12FeatureTests(unittest.TestCase):
    ...
    def _run_command(self, args: list[str], **kwargs: object) -> subprocess.CompletedProcess[str]:
    ...
    def test_run_real_rehearsal_wrapper_emits_triage_artifacts_when_blocked(self) -> None:
        run = self._run_powershell(
            self.project_root / "scripts" / "run_real_rehearsal.ps1",
            "-RunId",
    ...
    def test_run_phase12_iteration_wrapper_can_bootstrap_workspace_and_stop(self) -> None:
        ...
        run = self._run_powershell(
            self.project_root / "scripts" / "run_phase12_iteration.ps1",
            "-RunId",
    ...
    def test_run_phase12_iteration_wrapper_missing_working_dir_emits_operator_case_and_stub_plan(self) -> None:
        ...
        self.assertTrue((report_data / "phase12_apply_reconciliation.json").exists())
        self.assertTrue((report_data / "phase12_evidence_pack.json").exists())
        self.assertTrue((report_data / "phase12_real_gate.json").exists())
        self.assertFalse((report_data / "release_readiness.json").exists())
    ...
    def test_phase12_operator_case_includes_real_gate_summary(self) -> None:
        workspace_root = self.output_base / "operator-case-gate"
        run_root = self.output_base / "operator-case-gate-run"
        report_data = run_root / "report_data"
        incoming_root = workspace_root / "incoming"
        working_dir = workspace_root / "manual-drafts"
        live_root = self.output_base / "operator-case-live"
        live_manual_dir = live_root / "manual"
        incoming_root.mkdir(parents=True)
        working_dir.mkdir(parents=True)
        live_manual_dir.mkdir(parents=True)
        report_data.mkdir(parents=True)
        (report_data / "phase12_real_gate.json").write_text(
            json.dumps(
                {
                    "status": "blocked",
                    "hard_facts": {
                        "preflight": {"fresh_preflight_generated": True},
                        "remaining_blockers": ["real scan inputs are not ready"],
                        "warnings": ["phase12_evidence_pack.json is missing"],
                        "mismatch_summary": [],
                    },
                    "inference": {"rehearsal_allowed": False, "real_explicit_proof_status": "operator-confirmation-needed"},
                }
            ),
            encoding="utf-8",
        )
        ...
        self.assertEqual(run.returncode, 0, run.stderr)
        operator_case = json.loads((report_data / "phase12_operator_case.json").read_text(encoding="utf-8"))
        self.assertEqual(operator_case["hard_facts"]["real_gate_summary"]["status"], "blocked")
        self.assertFalse(operator_case["hard_facts"]["real_gate_summary"]["rehearsal_allowed"])
    ...
    def test_phase12_real_gate_reports_ready_for_rehearsal_with_real_explicit_proof(self) -> None:
        workspace_root, run_root, live_root = self._prepare_real_gate_ready_run("phase12-real-gate-ready")
        report_data = run_root / "report_data"
        ...
        run = self._run_command(
            [
                sys.executable,
                "-m",
                "vuln_pipeline.cli.phase12_real_gate",
                "--run-root",
                str(run_root),
                "--workspace-root",
                str(workspace_root),
        ...
        self.assertEqual(run.returncode, 0, run.stderr)
        gate = json.loads((report_data / "phase12_real_gate.json").read_text(encoding="utf-8"))
        self.assertEqual(gate["status"], "ready_for_rehearsal")
        self.assertTrue(gate["inference"]["rehearsal_allowed"])
        self.assertEqual(gate["inference"]["real_explicit_proof_status"], "hard_fact_real_explicit")

    def test_phase12_real_gate_reports_no_live_apply_detected_without_receipts(self) -> None:
        workspace_root, run_root, live_root = self._prepare_real_gate_ready_run("phase12-real-gate-no-apply", include_receipts=False)
        report_data = run_root / "report_data"
        ...
        self.assertNotEqual(run.returncode, 0)
        gate = json.loads((report_data / "phase12_real_gate.json").read_text(encoding="utf-8"))
        self.assertEqual(gate["status"], "no_live_apply_detected")
        self.assertFalse(gate["inference"]["rehearsal_allowed"])

    def test_phase12_real_gate_blocks_when_receipt_target_mismatches_current_live(self) -> None:
        workspace_root, run_root, live_root = self._prepare_real_gate_ready_run(
            "phase12-real-gate-mismatch",
            receipt_target_mismatch=True,
        )
        report_data = run_root / "report_data"
        ...
        self.assertTrue(any("receipt target_path" in item for item in gate["hard_facts"]["remaining_blockers"]))

    def test_phase12_real_gate_blocks_when_existing_preflight_is_stale(self) -> None:
        workspace_root, run_root, live_root = self._prepare_real_gate_ready_run("phase12-real-gate-stale")
        report_data = run_root / "report_data"
        stale_path = live_root / "burp" / "burp-stale.xml"
        stale_path.write_text("<issues>" + ("Z" * 350) + "</issues>", encoding="utf-8")
        (report_data / "input_preflight.json").write_text(
            json.dumps(
                {
                    "status": "ready",
                    "blocker_count": 0,
                    "blockers": [],
                    "manual_sources_ready": True,
                    "tool_checks": {
                        "burp": {"selected_files": [str(stale_path)]},
                        "nuclei": {"selected_files": [str(live_root / 'nuclei' / 'nuclei-real.jsonl')]},
                        "httpx": {"selected_files": [str(live_root / 'httpx' / 'httpx-real.jsonl')]},
                    },
                    "manual_inputs": {
                        "override_file": {"manual_source": "real_explicit", "effective_path": str(live_root / "manual" / "customer_override.yaml")},
                        "suppression_file": {"manual_source": "real_explicit", "effective_path": str(live_root / "manual" / "customer_suppressions.yaml")},
                        "review_resolution_file": {"manual_source": "real_explicit", "effective_path": str(live_root / "manual" / "customer_review_resolution.yaml")},
                    },
                }
            ),
            encoding="utf-8",
        )
        (report_data / "real_input_readiness.json").write_text(
            json.dumps({"status": "ready", "blocker_count": 0, "blockers": [], "manual_support": {}}),
            encoding="utf-8",
        )
        (report_data / "real_input_selection.json").write_text(
            json.dumps(
                {
                    "status": "selected",
                    "tools": {
                        "burp": {"selected_path": str(stale_path)},
                        "nuclei": {"selected_path": str(live_root / "nuclei" / "nuclei-real.jsonl")},
                        "httpx": {"selected_path": str(live_root / "httpx" / "httpx-real.jsonl")},
                    },
                    "manual_resolution": {
                        "override_file": {"manual_source": "real_explicit", "effective_path": str(live_root / "manual" / "customer_override.yaml")},
                        "suppression_file": {"manual_source": "real_explicit", "effective_path": str(live_root / "manual" / "customer_suppressions.yaml")},
        ...
        (report_data / "scan_promotion" / "live_scan_inventory.json").write_text(
            json.dumps(
                {
                    "status": "ready",
                    "hard_facts": {"active_valid_tool_count": 3, "tool_count": 3},
                    "inference": {"operational_goal_met": True},
                    "tools": {
                        "burp": {
                            "active_file": str(live_root / "burp" / "burp-real.xml"),
                            "eligible_file_count": 1,
                            "warnings": [],
                            "blockers": [],
                        },
                        "nuclei": {
                            "active_file": str(live_root / "nuclei" / "nuclei-real.jsonl"),
                            "eligible_file_count": 1,
                            "warnings": [],
                            "blockers": [],
                        },
                        "httpx": {
                            "active_file": str(live_root / "httpx" / "httpx-real.jsonl"),
                            "eligible_file_count": 1,
                            "warnings": [],
                            "blockers": [],
                        },
        ...
        gate = json.loads((report_data / "phase12_real_gate.json").read_text(encoding="utf-8"))
        self.assertEqual(gate["status"], "blocked")
        self.assertTrue(any("input_preflight selected file" in item for item in gate["hard_facts"]["remaining_blockers"]))

    def test_run_phase12_iteration_wrapper_stops_after_real_gate_and_uses_utf8_output(self) -> None:
        live_manual_dir = self.output_base / "live-manual-gate-only"
        live_root = self.output_base / "live-root-gate-only"
        incoming_root = self.output_base / "incoming-gate-only"
        working_dir = self.output_base / "working-gate-only"
        output_base = self.output_base / "wrapper-gate-only-runs"
        live_manual_dir.mkdir()
        for tool in ("burp", "nuclei", "httpx"):
            (live_root / tool).mkdir(parents=True)
            (incoming_root / tool).mkdir(parents=True)
        ...
        self.assertNotEqual(run.returncode, 0)
        report_data = output_base / "phase12-gate-only" / "report_data"
        self.assertTrue((report_data / "phase12_real_gate.json").exists())
        self.assertTrue((report_data / "phase12_operator_case.json").exists())
        self.assertTrue((report_data / "phase12_evidence_pack.json").exists())
        self.assertFalse((report_data / "release_readiness.json").exists())
        self.assertIn("Real-gate-only flow completed.", run.stdout)
        self.assertIn("Rehearsal execution", run.stdout)
        self.assertNotIn("�", run.stdout + run.stderr)
    ...
    def test_phase12_apply_reconciliation_reports_no_apply_for_blocked_run(self) -> None:
        blocked_run = self.project_root.parents[1] / "outputs" / "runs" / "phase12-real-rehearsal"
        if not blocked_run.exists():
            self.skipTest("phase12-real-rehearsal run is not available")

        out_dir = self.output_base / "blocked-reconciliation"
        run = subprocess.run(
            [
                sys.executable,
                "-m",
                "vuln_pipeline.cli.phase12_apply_reconciliation",
                "--run-root",
                str(blocked_run),
                "--output-dir",
                str(out_dir),
        ...
        self.assertEqual(run.returncode, 0, run.stderr)
        report = json.loads((out_dir / "phase12_apply_reconciliation.json").read_text(encoding="utf-8"))
        self.assertEqual(report["status"], "no_apply_detected")
        self.assertFalse(report["hard_facts"]["apply_detected"])
        self.assertTrue((out_dir / "phase12_apply_reconciliation.md").exists())

    def test_phase12_apply_reconciliation_marks_reconciled_when_receipts_match_live(self) -> None:
        workspace_root = self.output_base / "reconcile-workspace"
        run_root = workspace_root / "outputs" / "runs" / "phase12-reconcile"
        report_data = run_root / "report_data"
        scan_dir = report_data / "scan_promotion"
        manual_dir = report_data / "manual_promotion"
        live_root = workspace_root / "data" / "inputs" / "real"
        manual_live_dir = live_root / "manual"
        archive_dir = scan_dir / "archive" / "burp" / "20260316T000000Z"
        backup_dir = manual_dir / "backups"
        archive_dir.mkdir(parents=True)
        backup_dir.mkdir(parents=True)
        (live_root / "burp").mkdir(parents=True)
        (live_root / "nuclei").mkdir(parents=True)
        (live_root / "httpx").mkdir(parents=True)
        manual_live_dir.mkdir(parents=True)

        live_burp = live_root / "burp" / "burp-real.xml"
        incoming_burp = workspace_root / "incoming" / "burp-real.xml"
        incoming_burp.parent.mkdir(parents=True)
        live_burp.write_text("<issues>" + ("A" * 400) + "</issues>", encoding="utf-8")
        incoming_burp.write_text(live_burp.read_text(encoding="utf-8"), encoding="utf-8")
        archived_burp = archive_dir / "burp-real.xml"
        archived_burp.write_text("<issues>" + ("B" * 300) + "</issues>", encoding="utf-8")

        live_override = manual_live_dir / "customer_override.yaml"
        live_override.write_text("overrides:\n- issue_id: I-1\n", encoding="utf-8")
        override_backup = backup_dir / "customer_override.yaml.20260316T000000Z.bak"
        override_backup.write_text("overrides: []\n", encoding="utf-8")
            ...
            json.dumps(
                {
                    "hard_facts": {
                        ...
                        "live_root": {"path": str(live_root)},
                        "live_manual_dir": {"path": str(manual_live_dir)},
                    }
                }
            ),
            encoding="utf-8",
        )
        (report_data / "phase12_signoff_review.json").write_text(json.dumps({"status": "review_required"}), encoding="utf-8")
        (report_data / "phase12_apply_intent.json").write_text(json.dumps({"run_id": run_root.name}), encoding="utf-8")
        (report_data / "phase12_apply_intent_validation.json").write_text(json.dumps({"status": "valid", "block_reasons": []}), encoding="utf-8")
        (report_data / "phase12_evidence_pack.json").write_text(json.dumps({"status": "captured"}), encoding="utf-8")
        (report_data / "post_run_triage.json").write_text(
            json.dumps({"rollup_status": "pass", "hard_facts": {"blockers": []}}),
            encoding="utf-8",
        )
        (report_data / "input_preflight.json").write_text(
            json.dumps(
                {
                    "tool_checks": {
                        "burp": {"selected_files": [str(live_burp)]},
                        "nuclei": {"selected_files": []},
                        "httpx": {"selected_files": []},
                    },
                    "manual_inputs": {
                        "override_file": {"manual_source": "real_explicit"},
                    },
                }
            ),
            encoding="utf-8",
        ...
        (report_data / "real_input_selection.json").write_text(
            json.dumps(
                {
                    "tools": {"burp": {"selected_path": str(live_burp)}},
                    "manual_resolution": {"override_file": {"effective_path": str(live_override)}},
                }
            ),
            encoding="utf-8",
        )
            ...
            json.dumps(
                {
                    ...
                    "manual_support": {
                        "override_file": {
                            ...
                            "effective_execution_path": str(live_override),
                        }
                    },
                }
            ),
        ...
        (scan_dir / "scan_input_promotion_receipt.json").write_text(
            json.dumps(
                {
                    "status": "applied",
                    "applied_tools": {
                        "burp": {
                            "source_path": str(incoming_burp),
                            "target_path": str(live_burp),
                            "after_hash": self._sha256_path(live_burp),
                            "archived_files": [{"archive_path": str(archived_burp)}],
                        }
                    },
                }
            ),
        ...
        (manual_dir / "manual_promotion_receipt.json").write_text(
            json.dumps(
                {
                    "status": "applied",
                    "applied_files": [
                        {
                            "key": "override_file",
                            "after_path": str(live_override),
                            "after_hash": self._sha256_path(live_override),
                            "backup_path": str(override_backup),
                            "before_hash": self._sha256_path(override_backup),
                        }
                    ],
                }
            ),
        ...
        (report_data / "manual_validation.json").write_text(
            json.dumps(
                {
                    "rerun_live_context": {
                        "files": {
                            "override_file": {"path": str(live_override)},
        ...
        self.assertEqual(run.returncode, 0, run.stderr)
        report = json.loads((report_data / "phase12_apply_reconciliation.json").read_text(encoding="utf-8"))
        self.assertEqual(report["status"], "reconciled")
        self.assertTrue(report["hard_facts"]["apply_detected"])
        self.assertEqual(
            report["hard_facts"]["scan_reconciliation"]["burp"]["current_live_active_path"],
            str(live_burp),
        )

    def test_phase12_rollback_plan_and_apply_restore_temp_live_files(self) -> None:
        workspace_root = self.output_base / "rollback-workspace"
        run_root = workspace_root / "outputs" / "runs" / "phase12-rollback"
        report_data = run_root / "report_data"
        scan_dir = report_data / "scan_promotion"
        manual_dir = report_data / "manual_promotion"
        live_root = workspace_root / "data" / "inputs" / "real"
        ...
        live_burp = live_root / "burp" / "burp-real.xml"
        live_burp.write_text("<issues>" + ("N" * 320) + "</issues>", encoding="utf-8")
        restore_burp = archive_dir / "burp-real.xml"
        restore_burp.write_text("<issues>" + ("O" * 260) + "</issues>", encoding="utf-8")
        live_override = manual_live_dir / "customer_override.yaml"
        live_override.write_text("overrides:\n- issue_id: I-new\n", encoding="utf-8")
        restore_override = backup_dir / "customer_override.yaml.20260316T000000Z.bak"
        restore_override.write_text("overrides: []\n", encoding="utf-8")
            ...
            json.dumps(
                {
                    ...
                    "applied_tools": {
                        "burp": {
                            "target_path": str(live_burp),
                            "after_hash": self._sha256_path(live_burp),
                            "before_hash": self._sha256_path(restore_burp),
                            "archived_files": [{"archive_path": str(restore_burp)}],
                        }
                    },
                }
            ),
                    ...
                    "applied_files": [
                        {
                            ...
                            "after_path": str(live_override),
                            "after_hash": self._sha256_path(live_override),
                            "before_hash": self._sha256_path(restore_override),
                            "backup_path": str(restore_override),
                        }
                    ],
                }
            ),
        ...
        plan_dir = self.output_base / "rollback-plan-output"
        plan_run = subprocess.run(
            [
                sys.executable,
                "-m",
                "vuln_pipeline.cli.phase12_rollback",
                "--run-root",
                str(run_root),
                "--workspace-root",
                str(workspace_root),
        ...
        self.assertTrue((plan_dir / "phase12_rollback_plan.json").exists())
        self.assertIn("N" * 50, live_burp.read_text(encoding="utf-8"))

        apply_dir = self.output_base / "rollback-apply-output"
        apply_run = subprocess.run(
            [
                sys.executable,
                "-m",
        ...
        receipt = json.loads((apply_dir / "phase12_rollback_receipt.json").read_text(encoding="utf-8"))
        self.assertEqual(receipt["status"], "applied")
        self.assertEqual(live_burp.read_text(encoding="utf-8"), restore_burp.read_text(encoding="utf-8"))
        self.assertEqual(live_override.read_text(encoding="utf-8"), restore_override.read_text(encoding="utf-8"))

    def test_run_phase12_rollback_wrapper_blocks_without_valid_receipt(self) -> None:
        workspace_root = self.output_base / "rollback-wrapper-workspace"
        run_root = workspace_root / "outputs" / "runs" / "phase12-no-receipt"
        (run_root / "report_data").mkdir(parents=True)

        run = self._run_powershell(
            self.project_root / "scripts" / "run_phase12_rollback.ps1",
            "-RunRoot",
            str(run_root),
            "-WorkspaceRoot",
            str(workspace_root),
        ...
        self.assertNotEqual(run.returncode, 0)
        plan = json.loads((run_root / "report_data" / "phase12_rollback_plan.json").read_text(encoding="utf-8"))
        self.assertEqual(plan["status"], "blocked")

    def test_phase12_evidence_pack_includes_reconciliation_and_rollback_paths(self) -> None:
        run_root = self.output_base / "evidence-run"
        report_data = run_root / "report_data"
        delivery_dir = run_root / "delivery"
        report_data.mkdir(parents=True)
        delivery_dir.mkdir(parents=True)
        (report_data / "post_run_triage.json").write_text(
            json.dumps({"rollup_status": "blocked", "hard_facts": {"missing_artifacts": [], "blockers": []}}),
            encoding="utf-8",
        )
        (report_data / "manual_validation.json").write_text(json.dumps({"status": "invalid"}), encoding="utf-8")
        (report_data / "input_preflight.json").write_text(json.dumps({"status": "blocked", "tool_checks": {}}), encoding="utf-8")
        (report_data / "real_input_selection.json").write_text(json.dumps({"status": "incomplete", "tools": {}}), encoding="utf-8")
        (report_data / "phase12_signoff_review.json").write_text(json.dumps({"status": "review_required"}), encoding="utf-8")
        (report_data / "phase12_apply_intent_validation.json").write_text(json.dumps({"status": "valid", "block_reasons": []}), encoding="utf-8")
        (report_data / "phase12_apply_reconciliation.json").write_text(json.dumps({"status": "no_apply_detected"}), encoding="utf-8")
        (report_data / "phase12_real_gate.json").write_text(
            json.dumps(
                {
                    "status": "blocked",
        ...
        (report_data / "phase12_rollback_plan.json").write_text(json.dumps({"status": "plan_ready"}), encoding="utf-8")
        (report_data / "phase12_rollback_receipt.json").write_text(json.dumps({"status": "applied"}), encoding="utf-8")
        (report_data / "scan_promotion").mkdir()
        (report_data / "scan_promotion" / "live_scan_inventory.json").write_text(
            json.dumps({"status": "ready", "tools": {"burp": {}, "nuclei": {}, "httpx": {}}, "hard_facts": {"active_valid_tool_count": 0}}),
            encoding="utf-8",
        )
        ...
        self.assertEqual(run.returncode, 0, run.stderr)
        pack = json.loads((report_data / "phase12_evidence_pack.json").read_text(encoding="utf-8"))
        self.assertEqual(pack["hard_facts"]["apply_reconciliation"]["status"], "no_apply_detected")
        self.assertEqual(pack["hard_facts"]["real_gate"]["status"], "blocked")
        self.assertEqual(pack["hard_facts"]["rollback_plan"]["status"], "plan_ready")
        self.assertEqual(pack["hard_facts"]["rollback_receipt"]["status"], "applied")
    ...
    def _prepare_real_gate_ready_run(
        self,
        run_id: str,
        *,
        include_receipts: bool = True,
        receipt_target_mismatch: bool = False,
    ) -> tuple[Path, Path, Path]:
        workspace_root = self.output_base / f"{run_id}-workspace"
        app_root = workspace_root / "app" / "vuln-pipeline"
        run_root = workspace_root / "outputs" / "runs" / run_id
        report_data = run_root / "report_data"
        scan_dir = report_data / "scan_promotion"
        manual_dir = report_data / "manual_promotion"
        live_root = workspace_root / "data" / "inputs" / "real"
        ...
        for path in [
            app_root / "configs" / "customer_bundles",
            app_root / "configs" / "branding",
            app_root / "configs" / "readiness",
            workspace_root / "data" / "inputs" / "manual",
            live_root / "burp",
            live_root / "nuclei",
            live_root / "httpx",
            live_manual_dir,
        ...
        live_burp = live_root / "burp" / "burp-real.xml"
        live_nuclei = live_root / "nuclei" / "nuclei-real.jsonl"
        live_httpx = live_root / "httpx" / "httpx-real.jsonl"
        live_override = live_manual_dir / "customer_override.yaml"
        live_suppressions = live_manual_dir / "customer_suppressions.yaml"
        live_resolution = live_manual_dir / "customer_review_resolution.yaml"

        live_burp.write_text("<issues>" + ("A" * 420) + "</issues>", encoding="utf-8")
        live_nuclei.write_text(('{"template-id":"nuclei","info":{"name":"n"}}' + (" " * 40) + "\n") * 4, encoding="utf-8")
        live_httpx.write_text(('{"input":"https://example.com","status-code":200}' + (" " * 40) + "\n") * 4, encoding="utf-8")
        live_override.write_text("overrides:\n- issue_id: I-1\n", encoding="utf-8")
        live_suppressions.write_text("suppressions:\n- cluster_key: c-1\n", encoding="utf-8")
        live_resolution.write_text("review_resolutions:\n- issue_id: I-1\n", encoding="utf-8")

        archived_burp = archive_dir / "burp-prev.xml"
        archived_burp.write_text("<issues>" + ("B" * 320) + "</issues>", encoding="utf-8")
        override_backup = backup_dir / "customer_override.yaml.20260316T000000Z.bak"
        suppression_backup = backup_dir / "customer_suppressions.yaml.20260316T000000Z.bak"
        resolution_backup = backup_dir / "customer_review_resolution.yaml.20260316T000000Z.bak"
        override_backup.write_text("overrides: []\n", encoding="utf-8")
        suppression_backup.write_text("suppressions: []\n", encoding="utf-8")
        resolution_backup.write_text("review_resolutions: []\n", encoding="utf-8")
            ...
            json.dumps(
                {
                    ...
                    "hard_facts": {
                        "workspace_root": str(workspace_root),
                        "working_dir": {"path": str(workspace_root / "notes" / "manual-drafts"), "exists": False},
                        "incoming_root": {"path": str(workspace_root / "notes" / "incoming"), "exists": False},
                        "live_root": {"path": str(live_root), "exists": True},
                        "live_manual_dir": {"path": str(live_manual_dir), "exists": True},
                    },
        ...
        (report_data / "phase12_signoff_review.json").write_text(json.dumps({"status": "review_required"}), encoding="utf-8")
        (report_data / "phase12_apply_intent.json").write_text(json.dumps({"run_id": run_id}), encoding="utf-8")
        (report_data / "phase12_apply_intent_validation.json").write_text(
            json.dumps({"status": "valid", "block_reasons": []}),
            encoding="utf-8",
        )
        (report_data / "manual_validation.json").write_text(
            json.dumps(
                {
                    "status": "valid",
                    "format_valid_for_rerun": True,
                    "rerun_live_context": {
                        "format_valid": True,
                        "files": {
                            "override_file": {"path": str(live_override)},
                            "suppression_file": {"path": str(live_suppressions)},
                            "review_resolution_file": {"path": str(live_resolution)},
                        },
                    },
                }
            ),
        ...
        (report_data / "post_run_triage.json").write_text(
            json.dumps({"rollup_status": "pass", "hard_facts": {"missing_artifacts": [], "blockers": []}}),
            encoding="utf-8",
        )

        if include_receipts:
            receipt_target = live_burp if not receipt_target_mismatch else (live_root / "burp" / "burp-other.xml")
            (scan_dir / "scan_input_promotion_receipt.json").write_text(
                json.dumps(
                    {
                        "status": "applied",
                        "applied_tools": {
                            "burp": {
                                "source_path": str(workspace_root / "incoming" / "burp-real.xml"),
                                "target_path": str(receipt_target),
                                "after_hash": self._sha256_path(live_burp),
                                "archived_files": [{"archive_path": str(archived_burp)}],
                            },
                            "nuclei": {
                                "source_path": str(workspace_root / "incoming" / "nuclei-real.jsonl"),
                                "target_path": str(live_nuclei),
                                "after_hash": self._sha256_path(live_nuclei),
                                "archived_files": [],
                            },
                            "httpx": {
                                "source_path": str(workspace_root / "incoming" / "httpx-real.jsonl"),
                                "target_path": str(live_httpx),
                                "after_hash": self._sha256_path(live_httpx),
                                "archived_files": [],
                            },
                        },
                    }
                ...
                json.dumps(
                    {
                        ...
                        "applied_files": [
                            ...
                            {
                                "key": "suppression_file",
                                "after_path": str(live_suppressions),
                                "after_hash": self._sha256_path(live_suppressions),
                                "backup_path": str(suppression_backup),
                            },
                            {
                                "key": "review_resolution_file",
                                "after_path": str(live_resolution),
                                "after_hash": self._sha256_path(live_resolution),
                                "backup_path": str(resolution_backup),
                            },
                        ],
                    }
                ),
        ...
        return workspace_root, run_root, live_root
