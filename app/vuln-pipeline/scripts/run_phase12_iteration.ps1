...
param(
    ...
    [switch]$GenerateRealGate,
    [switch]$RequireRealGateForRehearsal,
    [switch]$StopAfterRealGate,
    [switch]$RefreshPreflightBeforeGate,
    [switch]$AllowRehearsalWithoutGatePass,
    [switch]$GenerateApplyReconciliation,
    [switch]$StopAfterReconciliation,
...
function Invoke-ApplyReconciliation {
    param(
        [string]$ResolvedWorkspaceRoot,
        [string]$ResolvedPreviousRunRoot,
        [string]$ResolvedIntentFile
    ...
    $args = @(
        "-m", "vuln_pipeline.cli.phase12_apply_reconciliation",
        "--run-root", $runRoot,
        "--output-dir", $reportDataDir,
        "--live-root", $LiveRoot,
        "--scan-receipt-dir", $ScanPromotionOutputDir,
        "--manual-receipt-dir", $PromotionOutputDir
    )
    if ($ResolvedWorkspaceRoot) {
        $args += "--workspace-root"
        $args += $ResolvedWorkspaceRoot
...
function Invoke-RealGate {
    ...
    # phase12_real_gate must inspect the repository live inputs, not the non-live operator workspace.
    $gateWorkspaceRoot = $repoWorkspaceRoot
    $args = @(
        "-m", "vuln_pipeline.cli.phase12_real_gate",
        "--run-root", $runRoot,
        "--workspace-root", $gateWorkspaceRoot,
        "--live-root", $LiveRoot,
        "--output-dir", $reportDataDir,
        "--json-out", $realGateJson,
        "--md-out", $realGateMd
    )
    if ($ResolvedPreviousRunRoot) {
        $args += "--previous-run-root"
    ...
    if ($ReadinessPolicy) {
        $args += "--readiness-policy"
        $args += $ReadinessPolicy
    }
    if ($RefreshPreflightBeforeGate) {
        $args += "--refresh-preflight"
    }
    if ($RequireRealGateForRehearsal -or -not $AllowRehearsalWithoutGatePass) {
        $args += "--strict"
    }
    return (Invoke-AppPython -Arguments $args)
}
...
$applyReconciliationJson = Join-Path $reportDataDir "phase12_apply_reconciliation.json"
$applyReconciliationMd = Join-Path $reportDataDir "phase12_apply_reconciliation.md"
$realGateJson = Join-Path $reportDataDir "phase12_real_gate.json"
$realGateMd = Join-Path $reportDataDir "phase12_real_gate.md"
...
$intentGateReasons = @()
$reconciliationRequested = $GenerateApplyReconciliation -or $ApplyScanPromotion -or $ApplyPromotion
$reconciliationExit = 0
$shouldGenerateRealGate = $GenerateRealGate -or $StopAfterRealGate -or -not $AllowRehearsalWithoutGatePass
$requireGatePassForRehearsal = $RequireRealGateForRehearsal -or -not $AllowRehearsalWithoutGatePass
$realGateExit = 0
$realGateStatus = "not_generated"
$realGateRehearsalAllowed = $false
$realGateSummaryLines = @()
$skipRehearsalForGate = $false
...
if ($reconciliationRequested) {
    $reconciliationExit = Invoke-ApplyReconciliation -ResolvedWorkspaceRoot $resolvedWorkspaceRoot -ResolvedPreviousRunRoot $resolvedPreviousRunRoot -ResolvedIntentFile $resolvedIntentFile
    Write-Host "Apply reconciliation artifacts:"
    Write-Host "  $applyReconciliationJson"
    Write-Host "  $applyReconciliationMd"
}

if ($StopAfterReconciliation) {
    $null = Write-OperatorCase -CasePhase "post-apply-reconciliation" -ResolvedWorkspaceRoot $resolvedWorkspaceRoot -ResolvedPreviousRunRoot $resolvedPreviousRunRoot
    $null = Invoke-EvidencePack -ResolvedWorkspaceRoot $resolvedWorkspaceRoot -ResolvedPreviousRunRoot $resolvedPreviousRunRoot -ResolvedIntentFile $resolvedIntentFile
    Write-Host "Post-apply reconciliation flow completed."
    Write-Host "Evidence artifacts:"
    Write-Host "  $evidencePackJson"
    Write-Host "  $evidencePackMd"
    if ($reconciliationExit -ne 0) { exit $reconciliationExit }
...
if ($shouldGenerateRealGate) {
    $realGateExit = Invoke-RealGate -ResolvedWorkspaceRoot $resolvedWorkspaceRoot -ResolvedPreviousRunRoot $resolvedPreviousRunRoot -ResolvedIntentFile $resolvedIntentFile
    Write-Host "Final real gate artifacts:"
    Write-Host "  $realGateJson"
    Write-Host "  $realGateMd"
    if (Test-Path $realGateJson) {
        $realGate = Get-Content -Raw -Encoding UTF8 $realGateJson | ConvertFrom-Json
        $realGateStatus = "$($realGate.status)"
        $realGateRehearsalAllowed = [bool]$realGate.inference.rehearsal_allowed
        $realGateSummaryLines = @(@($realGate.hard_facts.remaining_blockers) + @($realGate.hard_facts.warnings)) | Where-Object { $_ }
        Write-Host "  rollup: $realGateStatus"
        Write-Host "  rehearsal_allowed: $realGateRehearsalAllowed"
        if ($realGateSummaryLines.Count -gt 0) {
            Write-Host "  summary:"
            foreach ($line in $realGateSummaryLines) {
                Write-Host "    $line"
            }
        }
        Write-Host "  next: $($realGate.suggestions.rehearsal_command)"
        $skipRehearsalForGate = $requireGatePassForRehearsal -and -not $realGateRehearsalAllowed
    }
    else {
        $skipRehearsalForGate = $requireGatePassForRehearsal
        Write-Host "  real gate json is missing, so rehearsal gating stays blocked."
    }
}

if ($StopAfterRealGate) {
    $null = Write-OperatorCase -CasePhase "post-real-gate" -ResolvedWorkspaceRoot $resolvedWorkspaceRoot -ResolvedPreviousRunRoot $resolvedPreviousRunRoot
    $null = Invoke-EvidencePack -ResolvedWorkspaceRoot $resolvedWorkspaceRoot -ResolvedPreviousRunRoot $resolvedPreviousRunRoot -ResolvedIntentFile $resolvedIntentFile
    Write-Host "Rehearsal execution: skipped"
    Write-Host "  reason: stop-after-real-gate was requested"
    Write-Host "Real-gate-only flow completed."
    Write-Host "Evidence artifacts:"
    Write-Host "  $evidencePackJson"
    Write-Host "  $evidencePackMd"
    if ($realGateExit -ne 0) { exit $realGateExit }
    exit 0
}
...
$preflightExit = 0
$rehearsalExit = 0
if ($skipRehearsalForGate) {
    Write-Host "Rehearsal execution: skipped"
    Write-Host "  reason: final real gate did not allow rehearsal"
}
else {
    Write-Host "Rehearsal execution: running"
...
$finalCasePhase = if ($skipRehearsalForGate) { "post-real-gate" } else { "post-run" }
$null = Write-OperatorCase -CasePhase $finalCasePhase -ResolvedWorkspaceRoot $resolvedWorkspaceRoot -ResolvedPreviousRunRoot $resolvedPreviousRunRoot
$null = Invoke-EvidencePack -ResolvedWorkspaceRoot $resolvedWorkspaceRoot -ResolvedPreviousRunRoot $resolvedPreviousRunRoot -ResolvedIntentFile $resolvedIntentFile
...
Write-Host "  $evidencePackJson"
Write-Host "  $evidencePackMd"
Write-Host "  $(Join-Path $appRoot 'scripts\run_phase12_rollback.ps1')"
...
if ($manualPromotionExit -ne 0 -and $manualApplyAllowed) { exit $manualPromotionExit }
if ($reconciliationExit -ne 0) { exit $reconciliationExit }
if ($skipRehearsalForGate -and $requireGatePassForRehearsal) { exit $realGateExit }
