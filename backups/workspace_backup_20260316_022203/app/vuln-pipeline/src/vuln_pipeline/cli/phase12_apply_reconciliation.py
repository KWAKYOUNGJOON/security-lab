...
def _build_scan_tool_reconciliation(
    *,
    ...
    for tool in TOOL_ORDER:
        ...
        if not receipt_present and scan_receipt is None:
            missing.append(f"scan receipt missing for {tool}")
        output[tool] = {
...
def _build_manual_reconciliation(
    *,
    ...
    for key in MANUAL_KEY_ORDER:
        ...
        if not receipt_present and manual_receipt is None:
            missing.append(f"manual receipt missing for {key}")
        output[key] = {
            "receipt_present": receipt_present,
...
def _artifact_gaps(report_data: Path, intent_file: Path) -> list[str]:
    required_paths = {
        ...
        "phase12_evidence_pack.json": report_data / "phase12_evidence_pack.json",
