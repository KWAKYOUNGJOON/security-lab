...
apply 이후에는 receipt만 보지 말고 reconciliation을 기준 artifact로 본다.
...
- planned target vs current live
- current live vs preflight selected
- scan receipt source/target/hash vs current live hash
- manual receipt after_path/after_hash vs current live manual path/hash
- archive/backup 실제 존재 여부
- readiness blocker 잔존 여부
- manual source 가 `real_explicit`로 해석되었는지에 대한 hard fact 또는 bounded inference

wrapper apply 흐름에서는 `phase12_apply_reconciliation.json/md`가 자동 생성된다. `-StopAfterReconciliation`을 쓰면 apply 후 reconciliation/evidence까지만 보고 중단할 수 있다.

해석 원칙:

- hard fact
  - receipt 존재, hash/path 일치 여부, archive/backup 존재 여부
- inference
  - manual source 해석, follow-up summary
- suggestion
  - rerun command, rollback plan command, inspect path

## Rollback Plan And Apply

rollback은 항상 plan-first다.
...
rollback plan이 보여줘야 하는 것:

- restore 대상 파일 목록
- current live vs receipt before-state 차이
...
- current live 를 다시 백업한다
- restore source 가 없으면 자동 삭제/초기화하지 않고 block 한다
- receipt 에 restored target, source backup/archive, before/after hash, restore timestamp 를 남긴다

rollback 검토가 필요한 경우:

- apply 후 preflight/rehearsal 이 blocked/fail 로 바뀜
- wrong file promoted
- stale intent 기반 apply 발견
- reconciliation 이 `reconciliation_failed` 또는 강한 warning 을 표시함

## Handoff Evidence

handoff 시 reviewer/operator가 한 번에 봐야 하는 최소 artifact 순서는 아래다.
...
8. rollback 적용 후에는 `phase12_rollback_receipt.json/md`

evidence pack에서 추가로 볼 포인트:

- signoff review / intent validation status
- apply reconciliation rollup
- rollback plan / receipt path
- current live inventory 요약
- current preflight selection 요약
- rerun / rollback exact next command
...
rehearsal 직전에는 signoff / intent / apply / reconciliation 뒤에도 final real gate를 다시 본다. signoff는 reviewer 승인 상태를 확인하고, reconciliation은 apply receipt와 live state 정합을 확인하지만, final gate는 current live + fresh preflight + promotion receipt + manual `real_explicit` proof 가 동시에 수렴했는지를 한 번에 묶어서 판정한다.
...
- `manual_source=real_explicit` proof 또는 why-not-proven
- `input_preflight.status=ready`, blocker 0, `manual_sources_ready=true`
- intent validation valid/pass 여부
...
- 기본적으로 rehearsal 전에 `phase12_real_gate.json/md`를 만든다.
- gate가 `blocked` 또는 `no_live_apply_detected` 면 기본 동작으로 rehearsal을 실행하지 않는다.
- `-StopAfterRealGate` 로 gate-only 확인이 가능하다.
- blocked/pass/no-apply 와 관계없이 operator case / evidence pack 은 계속 남긴다.
...
5. gate pass 확인 후 rehearsal 진행
6. blocked 면 `missing_input_fix_command` 또는 `rollback_plan_command` 로 되돌아감

UTF-8 주의:

- PowerShell wrapper는 UTF-8 console encoding을 먼저 설정한다.
- wrapper smoke test는 `encoding="utf-8", errors="replace"` 로 실행해 cp949 decode warning을 줄인다.
